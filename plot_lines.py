...
def draw_lines(date):
    ...
    #get file paths
    cords_path = os.path.join(os.getcwd(),"Image_label_xmls") + "/" + "crosswalk_detections/" + date + "/person_cords.csv"
    timestamp_path = os.path.join(os.getcwd(),"Image_label_xmls") + "/" + "crosswalk_detections/" + date + "/frame_timestamps.csv"
    person_frames_path = os.path.join(os.getcwd(),"Image_label_xmls") + "/" + "crosswalk_detections/" + date + "/person_frames.csv"
    ...
    #track permaid, store tuples in list, loop through list, for master iamge containing all points
    for row in record:
        if(row[0] != perma_id):
            perma_id = row[0]
